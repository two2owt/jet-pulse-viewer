...
export const JetCard = memo(({ venue, onGetDirections, onClose }: JetCardProps) => {
  ...
  return (
    <article 
      className="bg-card rounded-2xl sm:rounded-3xl overflow-hidden border border-border shadow-[var(--shadow-card)] transition-all duration-300 hover-scale"
    ...
    >
      ...
      <div className="relative h-44 sm:h-52 md:h-60 lg:h-64 bg-gradient-to-br from-primary/20 via-accent/20 to-secondary/20 overflow-hidden">
        {venue.imageUrl ? (
          <OptimizedImage
            src={venue.imageUrl} 
            alt={venue.name}
