...
export const JetCard = memo(({ venue, onGetDirections, onClose }: JetCardProps) => {
  ...
  return (
    <article 
      className="relative bg-card rounded-3xl sm:rounded-[2rem] md:rounded-[2.5rem] overflow-hidden border border-border shadow-[var(--shadow-card)] transition-all duration-300 hover-scale"
    ...
    >
      ...
      {onClose && (
        <button
          onClick={onClose}
          className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 bg-background/90 backdrop-blur-md p-2 sm:p-2.5 rounded-full hover:bg-background transition-colors touch-manipulation"
          aria-label="Close"
        >
      ...
      <div className="relative h-36 sm:h-44 md:h-52 bg-gradient-to-br from-primary/20 via-accent/20 to-secondary/20 overflow-hidden rounded-t-3xl sm:rounded-t-[2rem] md:rounded-t-[2.5rem]">
        {venue.imageUrl ? (
          <OptimizedImage
            src={venue.imageUrl} 
            alt={venue.name}
        ...
        {/* Activity Badge */}
        <div className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-background/90 backdrop-blur-md px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-full flex items-center gap-1.5 sm:gap-2">
        ...
        {/* Category Badge */}
        <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 bg-muted/90 backdrop-blur-md px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-full">
          <span className="text-xs sm:text-sm font-semibold text-foreground">{venue.category}</span>
        </div>
      </div>

      {/* Content Area - No scroll for current content */}
      <div className="p-4 sm:p-5 md:p-6 space-y-4 sm:space-y-5">
        ...
        {/* Action Buttons */}
        <div className="grid grid-cols-2 gap-3 sm:gap-4 pt-1" role="group" aria-label="Venue actions">
          <Button 
            onClick={handleShare}
            variant="outline"
